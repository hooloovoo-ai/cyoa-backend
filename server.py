...
async def generate_api():
    ...
    if summarize:
        text_before_excerpt = args.get("text", "")[-500:]
...
async def imagine_api():
    ...
    if audio["duration"] <= 0:
        ...
        app.logger.info(f"Rendering audio in {len(texts)} parts using desired_length={desired_length} and max_length={max_length}")
        start = time.time()
        parts = group([tts.s(part) for part in texts]).apply_async().get()
        app.logger.info(f"Rendered audio in {time.time() - start} seconds")
        audio = combine_audio_convert_and_upload.delay(hash_of_text, parts).get()
